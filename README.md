...
完成影片以精簡卡片列出，可直接下載 MP4，或按「播放集錦」載入播放器；分享與單分檔案放在播放器內，原片下載、重新製作與刪除收在「更多操作」。重新整理後播放器回到收合狀態，避免多支成品同時消耗手機流量。

頁首連線狀態會在確認服務回應後才顯示「服務運作中」。裝置離線、服務無回應或檢查逾時會顯示對應狀態；影片進度讀取失敗時保留上次資料並標示尚未更新，恢復連線後自動更新。此指示確認網站可連線，不代表 GPU 或每支影片的處理結果正常。
...
在桌面「逐分標記」選擇影片並按「開始／繼續標記」。管理員可選擇所有 owner 的已完成影片，清單顯示擁有者；一般使用者的「我的影片」隔離契約不變。原片透過既有授權 HTTP Range 播放，不另切 MP4。

1. 第一次開啟時按「載入候選，開始檢查」，或「＋ 新增漏分」。候選只用核心起訖，模型精彩度分數不顯示。點選依時間排列的回合可直接預覽；前後留白只影響播放。
2. 左側看原片與選回合，右側依序操作：**① 確認完整回合 → ② 選精彩度 → ③ 儲存並下一分**。一分是發球到得分結束的完整回合；用 `I` / `O` 或秒數欄修正邊界，再按「確認：這是完整的一分」。這個明確操作同時設為 valid／confirmed；不是回合、看不清或仍待修正仍可用各自選項表達。
3. 精彩度按 `0`–`3`：普通／有看點／精彩／特別精彩；「未評分」與「無法判斷」另存。「不是回合」獨立於普通的一分。選中的分數會亮起；原因複選、品質問題與備註收在選填區。
4. `Enter` 儲存並下一分，`Space` 播放、方向鍵前後 1 秒（Shift 為 5 秒）、`N` 新增；輸入欄、按鈕及中文組字時停用快捷鍵。可篩選未評分、邊界待確認或已完成。
5. 「候選切錯了？」收合區提供拆分／合併／刪除。拆分使用目前播放位置；合併使用時間排序中相鄰的下一分。原 ID 保留於歷史，新 ID 透過 parent/superseded 關聯連回來源，邊界與評分重置待確認。刪除只撤下標記，不刪原片；此處不會改動舊 highlight/exclude 原始紀錄。
6. 在「補查漏分與原片檢查範圍」可前往尚未檢查的原片區間；每段確實完整檢查後，再自行記錄起訖範圍。候選全部評完不代表找齊全片回合；未覆蓋區間仍是未知。
7. 「匯出與資料工具」可匯出 JSON／JSONL 或重新讀取伺服器資料。詳見 [標記語意與分析方式](docs/evaluation.md#逐分標記資料契約-v1)。

「已儲存至伺服器」才代表 SQLite 寫入完成。未儲存草稿按帳號／原片暫存於目前瀏覽器；重新開啟可恢復，但不會跨裝置同步，清除網站儲存也會刪除草稿。切換回合會先儲存；失敗保留內容，過期版本回傳衝突，不會偷偷覆寫。回應遺失可按「重試上次儲存」，不會重複新增。已儲存標記在重跑分析及輸出重建後保持不變。

**目前正式舊素材庫尚未完成相容性驗收。** 請先使用 [隔離合成預覽](docs/deployment.md#逐分工作區的隔離合成預覽)，不要把本分支直接指向正式 `data/state.sqlite3`。這是人工資料收集工具，不代表精彩度演算法或剪輯品質已通過驗收。
