...
目前仍是 operator-run 第一階段，尚未接到常駐背景 worker，也還沒有 restore/hydration 或安全釋放本機空間。素材庫會從 SQLite 顯示「僅本機／處理中／本機 + pCloud／僅 pCloud／失敗」等狀態；它不讓網站容器持有 pCloud credential，也不在每次篩選時掃描遠端。僅 pCloud 的片段仍可被找到，但取回功能完成前不能預覽或加入集錦。因此即使顯示 `verified`，現在也不要手動刪除 `data/uploads`、active clips 或 compilations；播放器與重跑仍使用本機檔案。完整命名、狀態與故障恢復規則見 [pCloud 長期保存方案](docs/storage.md#pcloud-長期保存方案第一階段已實作)。
...
桌面版會把所有來源、目前版本與歷史版本的已索引片段一次載入同一個素材庫，預設只顯示目前版本，避免舊片段讓畫面混亂。可用檔名／工作 ID 搜尋，或依素材版本、來源影片、拍攝日期、來源內相對分數、片段長度、本機 availability 與 pCloud archive 狀態篩選排序。清除條件會回到「目前版本 + 全部儲存狀態」，不會刪除或遺漏索引。Pixel 類型的 `PXL_YYYYMMDD_HHMMSS...` 檔名會解析成拍攝日期；無法解析時才使用加入系統的日期。
