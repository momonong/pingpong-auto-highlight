...
在桌面「人工標記」選擇影片並開啟工作區。管理員可選擇所有 owner 的已完成影片，清單顯示擁有者；一般使用者的「我的影片」隔離契約不變。原片透過既有授權 HTTP Range 播放，不另切 MP4。

1. 按「匯入目前候選與舊標記」載入提案，或「新增漏掉的一分」。候選只用核心起訖，模型精彩度分數不顯示。點選依時間排列的回合可直接預覽；前後留白只影響播放。
2. 一分是**發球到得分結束的完整回合**。在播放器用 `I` / `O` 設邊界，或輸入秒數（精確至毫秒），再明確設定回合有效性、邊界確認狀態。
3. 精彩度按 `0`–`3`：普通／有看點／精彩／特別精彩；「未評分」與「無法判斷」另存。「不是回合」獨立於普通的一分。原因複選、品質問題與備註皆選填。
4. `Enter` 儲存並下一分，`Space` 播放、方向鍵前後 1 秒（Shift 為 5 秒）、`N` 新增；輸入欄、按鈕及中文組字時停用快捷鍵。可篩選未評分、邊界待確認或已完成。
5. 拆分使用目前播放位置；合併使用時間排序中相鄰的下一分。原 ID 保留於歷史，新 ID 透過 parent/superseded 關聯連回來源，邊界與評分重置待確認。刪除只撤下標記，不刪原片；此處不會改動舊 highlight/exclude 原始紀錄。
6. 每段原片完整檢查後，另記錄「已完整檢查的原片範圍」。候選全部評完不代表找齊全片回合；未覆蓋區間仍是未知。
7. 使用 JSON 或 JSONL 匯出離線分析。詳見 [標記語意與分析方式](docs/evaluation.md#逐分標記資料契約-v1)。
