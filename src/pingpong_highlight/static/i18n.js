(() => {
  ...
  const catalog = Object.freeze({
    "flow.savedNext": ["上一分已儲存，正在檢查第 {number} 分。", "Previous point saved. Now reviewing point {number}."],
    "flow.saveStep": ["儲存這一分", "Save this point"],
    "flow.title": ["逐分標記", "Point review"],
    "flow.intro": ["選一支影片，確認每一分的起訖，再評定精彩程度。儲存後，標記會保留在這支原片下。", "Choose a video, check each point’s boundaries, then rate its excitement. Saved labels stay with the source."],
    "flow.adminTitle": ["帳號與資料管理", "Accounts and data"],
    "flow.adminLead": ["管理帳號、查看各使用者的影片與儲存狀態。", "Manage accounts, videos across owners, and storage."],
    "flow.choose": ["選擇要標記的影片", "Choose a video to review"],
    "flow.chooseHelp": ["從下方開啟影片。已有標記時會接續進度；沒有標記時，先載入候選或自己新增。", "Open a video below. Resume existing work, or load candidates and add points yourself."],
    "flow.stepChoose": ["選影片", "Choose a video"],
    "flow.stepReview": ["逐分確認與評分", "Check and rate each point"],
    "flow.stepCoverage": ["補查原片中的漏分", "Check the source for missing points"],
    "flow.open": ["開始／繼續標記", "Start / resume review"],
    "flow.practice": ["操作練習素材", "Practice footage"],
    "flow.practiceHelp": ["這段是合成練習影片，用來熟悉操作，沒有真實桌球回合。", "This synthetic practice video demonstrates the controls; it contains no real table-tennis points."],
    "flow.changeVideo": ["返回選片", "Back to videos"],
    "flow.list": ["影片中的回合", "Points in this video"],
    "flow.listHelp": ["點選一分播放；需要修正時，在右側調整。", "Select a point to play it, then make corrections on the right."],
    "flow.filter": ["顯示", "Show"],
    "flow.progress": ["已處理 {done} / {total} 筆", "Reviewed {done} / {total} entries"],
    "flow.number": ["第 {number} 分", "Point {number}"],
    "flow.newPoint": ["新增的一分", "New point"],
    "flow.noPoint": ["先選擇一分", "Select a point to begin"],
    "flow.loadTitle": ["先建立待檢查清單", "Prepare your review list"],
    "flow.loadHelp": ["候選是系統找出的可能回合，需要你逐一確認；舊標記會保留原內容，另行評分。也可以直接新增漏掉的一分。", "Candidates are possible points for you to check. Legacy labels keep their original content and need a separate rating. You can also add a missing point."],
    "flow.load": ["載入候選，開始檢查", "Load candidates and begin"],
    "flow.proposals": ["候選與舊標記來源", "Candidates and legacy labels"],
    "flow.importMore": ["載入尚未加入的候選／舊標記", "Load additional candidates / legacy labels"],
    "flow.new": ["＋ 新增漏分", "+ Add missing point"],
    "flow.watch": ["看完這一分，確認起訖", "Watch the point and check its boundaries"],
    "flow.watchHelp": ["起點是發球，終點是得分結束；前後預覽留白不算在標記裡。", "Start at the serve and end when the point ends. Preview padding is separate."],
    "flow.start": ["發球起點（秒）", "Serve start (seconds)"],
    "flow.end": ["得分終點（秒）", "Point end (seconds)"],
    "flow.validity": ["這段是回合嗎？", "Is this a rally?"],
    "flow.boundary": ["起訖完整嗎？", "Are the boundaries complete?"],
    "flow.confirm": ["確認：這是完整的一分", "Confirm: this is a complete point"],
    "flow.confirmed": ["已確認這是完整的一分", "Confirmed as a complete point"],
    "flow.rate": ["這一分有多精彩？", "How exciting was this point?"],
    "flow.rateHelp": ["普通的一分也要保留；無法判斷時不用勉強給分。", "Keep ordinary points too. If unsure, choose unable to judge."],
    "flow.unrated": ["暫不評分", "Leave unrated"],
    "flow.optional": ["原因、品質問題與備註（選填）", "Reasons, quality issues and notes (optional)"],
    "flow.structure": ["候選切錯了？拆分／合併／刪除", "Incorrect segment? Split / merge / delete"],
    "flow.structureHelp": ["拆分用播放位置；合併用時間上下一筆。新區間需重新確認與評分。", "Split at the playhead or merge with the next entry in time. Review and rate the new intervals again."],
    "flow.previewOptions": ["預覽設定與快捷鍵", "Preview settings and shortcuts"],
    "flow.nextHelp": ["確認、評分後按 Enter 儲存並下一分；未完成也可先儲存稍後繼續。", "After checking and rating, press Enter to save and move on. You can also save unfinished work."],
    "flow.hintStart": ["下一步：載入候選，或新增你找到的一分。", "Next: load candidates or add a point you found."],
    "flow.hintSelect": ["下一步：從左側選一分，開始檢查。", "Next: select a point from the list."],
    "flow.hintBoundary": ["下一步：播放這段，確認它是完整的一分。", "Next: play this segment and check that it is a complete point."],
    "flow.hintRating": ["下一步：選擇精彩程度，或標為無法判斷。", "Next: rate the excitement or choose unable to judge."],
    "flow.hintSave": ["下一步：儲存並前往下一分。", "Next: save and move to the next point."],
    "flow.hintNotRally": ["這筆不是回合；儲存後可繼續檢查下一筆。", "This entry is not a rally; save it and continue."],
    "flow.hintDone": ["清單已處理完。接著補查原片，確認是否還有漏掉的分。", "The list is reviewed. Now check the source for missing points."],
    "flow.coverageTitle": ["補查漏分與原片檢查範圍", "Missing points and source coverage"],
    "flow.coverageShort": ["清單評完 ≠ 原片檢查完", "Finishing the list does not finish source review"],
    "flow.coverageToggle": ["記錄／查看已檢查範圍", "Record / view checked intervals"],
    "flow.coverageStart": ["已檢查區段起點（秒）", "Checked interval start (seconds)"],
    "flow.coverageEnd": ["已檢查區段終點（秒）", "Checked interval end (seconds)"],
    "flow.coverageProgress": ["原片已檢查 {checked} / {total}；其餘仍未知。", "Source checked: {checked} / {total}; the rest is unknown."],
    "flow.coveragePlay": ["前往下一個未檢查區間", "Go to next unchecked interval"],
    "flow.data": ["匯出與資料工具", "Export and data tools"],
    "flow.exportHelp": ["匯出已儲存的標記與檢查範圍；不會產生獨立影片。", "Export saved labels and coverage; no separate clips are created."],
