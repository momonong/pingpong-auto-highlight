(() => {
  ...
  const catalog = Object.freeze({
    "point.loaded": ["已載入伺服器標記", "Server annotations loaded"],
    "point.retry": ["重試上次儲存", "Retry last save"],
    "point.title": ["逐分標記工作區", "Point review workspace"],
    "point.lead": ["從發球到得分結束，檢查完整的一分。候選不代表人工真值。", "Review each complete point from serve to scoring end. Candidates are proposals."],
    "point.range": ["原片時間（秒）", "Source time (seconds)"],
    "point.preview": ["預覽這一分", "Preview point"],
    "point.padding": ["預覽前後留白（秒；不改標記）", "Preview padding (seconds; does not change labels)"],
    "point.validity": ["回合有效性", "Point validity"],
    "point.pending": ["待確認", "Pending"],
    "point.valid": ["真正的回合", "Valid point"],
    "point.not_rally": ["不是回合", "Not a rally"],
    "point.unclear": ["看不清", "Unclear"],
    "point.boundary": ["邊界狀態", "Boundary status"],
    "point.confirmed": ["已人工確認", "Human confirmed"],
    "point.needs_adjustment": ["仍待修正", "Needs adjustment"],
    "point.rating": ["精彩度（0–3）", "Excitement (0–3)"],
    "point.unrated": ["未評分", "Unrated"],
    "point.unable": ["無法判斷", "Unable to judge"],
    "point.rated": ["已評分", "Rated"],
    "point.score0": ["0 · 普通，不會收錄", "0 · Ordinary; omit"],
    "point.score1": ["1 · 有看點，通常不優先", "1 · Interesting; low priority"],
    "point.score2": ["2 · 精彩，願意收錄", "2 · Exciting; include"],
    "point.score3": ["3 · 特別精彩，優先保留", "3 · Exceptional; prioritize"],
    "point.reasons": ["精彩原因（選填、複選）", "Reasons (optional, multiple)"],
    "point.quality": ["品質問題（與精彩原因分開）", "Quality issues (separate from reasons)"],
    "point.note": ["補充備註", "Notes"],
    "point.save": ["儲存", "Save"],
    "point.next": ["儲存並下一分", "Save and next"],
    "point.new": ["新增漏掉的一分", "Add missing point"],
    "point.split": ["在播放位置拆分", "Split at playhead"],
    "point.merge": ["與下一分合併", "Merge with next"],
    "point.delete": ["刪除此標記", "Delete point"],
    "point.list": ["依時間排列的回合", "Points in time order"],
    "point.all": ["全部", "All"],
    "point.complete": ["已完成", "Completed"],
    "point.boundaryPending": ["邊界待確認", "Boundary pending"],
    "point.import": ["匯入目前候選與舊標記", "Import current proposals and legacy labels"],
    "point.reload": ["重新讀取／放棄草稿", "Reload / discard draft"],
    "point.export": ["匯出 JSON", "Export JSON"],
    "point.exportLines": ["匯出 JSONL", "Export JSONL"],
    "point.coverage": ["已完整檢查的原片範圍", "Source intervals fully checked"],
    "point.coverageHelp": ["只記錄已逐段檢查、尋找所有回合的原片範圍。候選全評完不代表全片已檢查；其餘區間仍是未知。", "Record only intervals checked for all points. Finishing candidates does not establish full coverage; other intervals remain unknown."],
    "point.coverageAdd": ["確認此區間已完整檢查", "Confirm interval fully checked"],
    "point.coverageEmpty": ["尚未記錄覆蓋範圍；全片仍為未知。", "No coverage recorded; entire source is unknown."],
    "point.unknown": ["未知區間：{ranges}", "Unknown intervals: {ranges}"],
    "point.covered": ["已明確記錄全片覆蓋（不代表模型或標記正確率）", "Full coverage explicitly recorded (not a model or label accuracy claim)"],
    "point.remove": ["撤回此範圍", "Retract interval"],
    "point.legacy": ["舊 highlight/exclude 原始紀錄（不換算評分）", "Original highlight/exclude records (no inferred scores)"],
    "point.saving": ["儲存中…", "Saving…"],
    "point.saved": ["已儲存至伺服器", "Saved to server"],
    "point.draft": ["尚未儲存；草稿已暫存此瀏覽器", "Unsaved; draft retained in this browser"],
    "point.restored": ["已恢復此帳號的本機草稿；請確認後儲存。", "Restored this account’s local draft; review and save."],
    "point.failed": ["儲存失敗，保留草稿。{detail}", "Save failed; draft retained. {detail}"],
    "point.conflict": ["其他視窗已修改資料。草稿保留；請先記下差異，再重新讀取，避免覆寫。", "Another window changed this review. Draft retained; note your changes, then reload before editing."],
    "point.localFailed": ["無法暫存本機草稿，請先儲存至伺服器再離開。", "Local draft storage unavailable; save to server before leaving."],
    "point.discard": ["放棄此瀏覽器的未儲存草稿，重新讀取伺服器資料？", "Discard this browser’s unsaved draft and reload server data?"],
    "point.closeDraft": ["目前仍有未儲存內容。保留本機草稿並關閉？", "There are unsaved changes. Keep the local draft and close?"],
    "point.confirmDelete": ["刪除此標記？歷史與來源關聯仍會保留於匯出。", "Delete this point? History and source links remain in the export."],
    "point.confirmStructure": ["拆分／合併後新區間需重新確認邊界與評分；原標記留在歷史中。繼續？", "New segments need fresh boundary confirmation and ratings; originals remain in history. Continue?"],
    "point.confirmCoverage": ["確認你已檢查此整段原片並尋找所有回合？", "Have you checked this entire source interval for all points?"],
    "point.importHelp": ["可匯入 {count} 個提案，略過 {skipped} 個無效候選。模型分數不顯示；新分析批次須明確匯入，可能與既有標記重疊。", "{count} proposals available; {skipped} invalid candidates skipped. Model scores are hidden. New batches require explicit import and may overlap existing labels."],
    "point.imported": ["目前批次已匯入；再次匯入不會重複新增。", "Current batch imported; importing again will not duplicate it."],
    "point.noSelection": ["請先選擇一分或新增。", "Select or add a point first."],
    "point.noNext": ["已到清單末尾；請繼續檢查未覆蓋的原片區間。", "End of list; continue checking uncovered source intervals."],
    "point.empty": ["這個篩選沒有回合。可匯入候選或新增漏分。", "No points in this filter. Import proposals or add missing points."],
    "point.shortcuts": ["I / O 設邊界 · 0–3 評分 · Enter 儲存並下一分 · Space 播放 · N 新增（輸入欄內停用）", "I / O boundaries · 0–3 rating · Enter save and next · Space play · N new (disabled in inputs)"],
    "point.selection": ["目前：{id}", "Selected: {id}"],
    "point.automatic": ["自動提案", "Automatic proposal"],
    "point.manual": ["人工新增", "Manual"],
    "point.legacyOrigin": ["舊標記", "Legacy"],
    "point.splitOrigin": ["拆分", "Split"],
    "point.mergeOrigin": ["合併", "Merge"],
    "point.count": ["{count} 個回合 · {complete} 個完成；不代表全片覆蓋", "{count} points · {complete} completed; not full-source coverage"],
    ...
    "workspace.title": ["我的影片", "My videos"],
    "workspace.lead": ["從原片到精彩集錦，在這裡掌握每一步。", "Follow every step from footage to finished reel."],
    "workspace.add": ["新增影片", "Add video"],
    "workspace.library": ["影片工作區", "Video workspace"],
    "workspace.admin": ["管理", "Administration"],
    "workspace.annotate": ["人工標記", "Annotations"],
    "workspace.more": ["更多操作", "More actions"],
    "workspace.play": ["播放集錦", "Play reel"],
    "workspace.stale": ["進度尚未更新，以下為上次取得的資料。連線恢復後會自動更新。", "Progress is out of date. Showing the last received data; updates resume when connected."],
    "workspace.loading": ["正在讀取影片與進度…", "Loading videos and progress…"],
    "workspace.all": ["全部", "All"],
    "workspace.active": ["進行中", "In progress"],
    "workspace.ready": ["已完成", "Completed"],
    "workspace.failed": ["需處理", "Needs attention"],
    "workspace.noMatch": ["此分類目前沒有影片", "No videos in this category"],
    "workspace.resume": ["選擇原片續傳", "Select original to resume"],
