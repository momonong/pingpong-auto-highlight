...
(async () => {
  ...
  try {
    ...
    const expand = async id => { if (!await page.locator(`#${id}`).evaluate(e => e.open)) await page.locator(`#${id} > summary`).click(); };
    const open = async () => {
    ...
    assert(await page.locator('#pointProposals').evaluate(e => e.open));
    assert((await page.locator('.workspace-heading h1').innerText()).includes('逐分標記'));
    assert.equal(await page.locator('#addVideoButton').isVisible(), false);
    assert.equal(await page.locator('.process-strip').isVisible(), false);
    assert(await page.locator('#pointPractice').isVisible());
    ...
    assert.equal(await page.locator('#pointValidity').inputValue(), 'valid');
    assert.equal(await page.locator('#pointBoundary').inputValue(), 'confirmed');
    await expand('pointOptional');
    ...
    assert.equal(await page.locator('#pointRate0').getAttribute('aria-pressed'), 'false');
    assert.equal(await page.locator('#pointConfirm').getAttribute('aria-pressed'), 'false');
    const positions = await page.evaluate(() => { const b = id => document.getElementById(id).getBoundingClientRect(); return {playerRight:b('annotationWorkspaceVideo').right, editorLeft:b('annotationWorkspaceForm').left, nextBottom:b('pointNext').bottom, viewHeight:innerHeight}; });
    assert(positions.playerRight <= positions.editorLeft && positions.nextBottom < positions.viewHeight);
    await page.screenshot({path:path.join(dir,'guided-review.png'),fullPage:true});
    // Unknown and unable remain distinct from ordinary complete points.
    await page.locator('#pointConfirm').click();
    assert.equal(await page.locator('#pointValidity').inputValue(), 'valid');
    assert.equal(await page.locator('#pointBoundary').inputValue(), 'confirmed');
    await page.locator('#pointUnable').click();
    ...
    await page.locator('#pointRate3').click();
    await page.locator('#annotationWorkspaceSave').click(); await saved();
    ...
    await page.locator('#pointCoveragePanel > summary').click();
