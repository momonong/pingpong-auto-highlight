...
(async()=>{
 ...
 try{
  const ctx=await browser.newContext({ignoreHTTPSErrors:true,locale:'zh-TW',viewport:{width:1440,height:1000}});
  ...
  // Preserve the snapshot's actual review state; never assume zero human points.
